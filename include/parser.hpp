...
template <typename... Ts> struct aux {
  static tuple<Ts...> as_tuple(XMLParser &parser, attrs &attrs,
                               attrs::iterator a);
  static tuple<Ts...> as_tuple_opt(XMLParser &parser, attrs &attrs,
                                   attrs::iterator a);
};
...
private:
  XMLParser(shared_ptr<string> content, shared_ptr<xml_document<>> doc,
            xml_node<> *node);
};

template <typename T, typename... Ts> struct aux<T, Ts...> {
  static tuple<T, Ts...> as_tuple(XMLParser &parser, attrs &attrs,
                                  attrs::iterator a) {
    if (a == attrs.end())
      throw InvalidXMLStructure(
  ...
  static tuple<T, Ts...> as_tuple_opt(XMLParser &parser, attrs &attrs,
                                      attrs::iterator a) {
    if (a == attrs.end())
      throw InvalidXMLStructure(
...
template <> struct aux<> {
  static tuple<> as_tuple(__attribute__((unused)) XMLParser &, attrs &attrs,
                          attrs::iterator a) {
    if (a != attrs.end())
      throw InvalidXMLStructure(
  ...
  static tuple<> as_tuple_opt(__attribute__((unused)) XMLParser &, attrs &attrs,
                              attrs::iterator a) {
    if (a != attrs.end())
      throw InvalidXMLStructure(
